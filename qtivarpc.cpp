...
void QTivaRPC::ModeChanged()
{
    ...
    if(connected)
    {
        // Se crea la trama con n de secuencia 0; comando COMANDO_MODE; se le pasa la
        // estructura de parametros, indicando su tamaño; el nº final es el tamaño maximo
        // de trama
        size=create_frame((unsigned char *)paquete, COMMAND_MODE, NULL, 0, MAX_FRAME_SIZE);
        // Se se pudo crear correctamente, se envia la trama
        if (size>0) serial.write(paquete,size);
    }
}

void QTivaRPC::GPIOChanged()
{
    char paquete[MAX_FRAME_SIZE];
    int size;
    ...
    if(connected)
    {
        // Se crea la trama con n de secuencia 0; comando COMANDO_GPIO; se le pasa la
        // estructura de parametros, indicando su tamaño; el nº final es el tamaño maximo
        // de trama
        size=create_frame((unsigned char *)paquete, COMMAND_GPIO, NULL, 0, MAX_FRAME_SIZE);
        // Se se pudo crear correctamente, se envia la trama
        if (size>0) serial.write(paquete,size);
    }
}
