  ...
  , transactionCount(0)
{
    ...
    connect(ui->modeButton,SIGNAL(clicked(bool)),&tiva,SLOT(ModeChanged()));

    connect(ui->colorWheel,SIGNAL(colorChanged(QColor)),this,SLOT(on_colorWheel_colorChanged(QColor)));
    connect(ui->colorWheel,SIGNAL(colorSelected(QColor)),this,SLOT(on_colorWheel_colorChanged(QColor)));
}
...
void MainUserGUI::on_modeButton_clicked()
{
    tiva.ModeChanged();
}


void MainUserGUI::on_GPIOButton_clicked()
{
    tiva.GPIOChanged();
}
